    ...
    <>
      <main className="min-h-screen text-black w-full">
        <div className="container max-w-5xl mx-auto flex justify-between items-center pt-4  sticky top-0 z-50 bg-[#f3f3f2] ">
          <div className="flex justify-center items-center gap-1 rounded-t-3xl py-4 px-6 !m-0 relative">
            <div className={`absolute inset-0 bg-[#9ceea0] rounded-t-3xl transition-all duration-400 ease-in-out pb-10 ${scrolled ? "-translate-y-[-100%] opacity-0" : "translate-y-0 opacity-100"
              }`}></div>
        ...
        <section className="w-full text-center px-6 relative overflow-hidden">
          <div className="max-w-5xl mx-auto bg-[#9ceea0] rounded-b-3xl rounded-tr-3xl py-20 px-10 relative">
            <div
              className={`absolute top-0 left-0 w-full h-[72px] bg-[#9ceea0] rounded-t-3xl transition-all duration-500 ${scrolled ? "translate-y-[-100%]" : "translate-y-0"
                }`}
            ></div>
              ...
              <div className="space-y-4 text-left">
                ...
                <div className="flex justify-start">
                  <div className="whitespace-pre-wrap leading-relaxed text-sm shadow-md  text-black px-4 py-3 rounded-2xl max-w-[75%] opacity-0 animate-fade-in [animation-delay:3000ms]">
                    Your current USDC balance on Ethereum is 42.00 USDC
                  </div>
                </div>
                ...
                <div className="flex justify-end">
                  <div className="whitespace-pre-wrap bg-[#2b4bbf] leading-relaxed text-sm shadow-md px-4 py-3 rounded-2xl max-w-[75%] opacity-0 text-white animate-fade-in [animation-delay:3000ms]">
                    I want to bridge 10 USDC to Rootstock
                  </div>
                </div>
                ...
                <div className="flex justify-start">
                  <div className="whitespace-pre-wrap leading-relaxed text-sm shadow-md  text-black px-4 py-3 rounded-2xl max-w-[75%] opacity-0 animate-fade-in [animation-delay:3000ms]">
                    Your bridge transaction has been successfully submitted! tx hash: 0x1234567890abcdef
                    It can take up to 10 minutes to complete. You can track the status of your transaction on LayerZero Explorer.
                  </div>
                </div>
              </div>
            </div>
